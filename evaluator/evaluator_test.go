...
func TestStringLiteral(t *testing.T) {
	...
	if str.Value != "Hello World!" {
		t.Errorf("String has wrong value. expected 'Hello World!' got %q", str.Value)
	}
}
...
func TestBuiltinFunctions(t *testing.T) {
	tests := []struct {
		...
		{`typeof("one", "two")`, "wrong number of arguments. expected 1, got 2"},
		{`typeof(6)`, "INTEGER"},
		...
		case string:
			...
			if ok {
				if str.Value != tt.expected {
					t.Errorf("wrong string value. expected %q got %q", tt.expected, str.Value)
				}
				continue
			}
...
func TestArrayLiterals(t *testing.T) {
	...
	if len(result.Elements) != 3 {
		t.Fatalf("array has not 3 elements. got %d", len(result.Elements))
	}
